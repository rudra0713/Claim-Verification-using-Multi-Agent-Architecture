...
from prompts.autogen_prompts_3_agents_v14 import *
import argparse
import time
import json
import logging
...
if __name__ == '__main__':
    ...
    error_ids = []
    tables_to_read = read_selected_table(error_ids if args.only_error_ids else [], threshold=int(args.number_of_claims), only_allow_unique_tables=args.only_allow_unique_tables)
    ...
    for batch_start in range(0, len(tables_to_read), BATCH_SIZE):
        ...
        for index, table_read in enumerate(batch_tables):
            ...
            message = f"""Please verify this claim: {claim} Using the table: {sample_data}, the table caption: {table_caption}, and the column headers: {col_headers}
        ...
        output_file_name = f'prediction_v14_{args.dataset_name}_{str(len(tables_to_read))}_claims_{args.model.replace("/", "")}.json'
